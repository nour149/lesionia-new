...
public class CustomerAuthentificationFilter extends UsernamePasswordAuthenticationFilter {
    ...
    protected void successfulAuthentication(HttpServletRequest request,
        ...
        String acces_token= JWT.create()
                ...
                .withExpiresAt(new Date(System.currentTimeMillis() + 100 * 600 *1080))
        ...
        String refresh_token= JWT.create()
                .withSubject(user.getUsername())
                .withExpiresAt(new Date(System.currentTimeMillis()+ 30 *60 * 1080))
                .withIssuer(request.getRequestURL().toString())
                .withClaim("roles" , user.getAuthorities().stream().map(GrantedAuthority ::getAuthority).collect(Collectors.toList()))
                .sign(algorithm);
...
*/
        /*token in cookie*/
        Cookie cookie = new Cookie("coo",acces_token);
